...
function HabitCategoryGroup({ habits, deleteHabit, onToggle, categoryOptions, handleViewNote,handleAddNote }) {
    return (
                                ...
                                <CardContent sx={{ overflowY: 'auto', flexGrow: 1 }}>
                                    {filteredHabits.map((hab) => (
                                        <div key={hab._id}>
                                            <Checkbox
                                                checked={hab.checked}
                                                onChange={() => onToggle(hab._id,hab.checked)} />
                                            {hab.text}
                                        ...
                                        <Button onClick={()=>handleViewNote(hab)}>📝</Button> 
                                            ...
                                            {hab.note ? (
                                                <Button onClick={() => handleViewNote(hab)}>View Note 📝</Button>
                                            ) : (
                                                <Button onClick={() => handleAddNote(hab)}>Add Note ➕</Button>
                                            )}
